...
func (s *S) TestHealersFromResource(c *C) {
	...
	os.Setenv("TSURU_TOKEN_OWNER", "tsuru-healer")
	defer os.Setenv("TSURU_TOKEN_OWNER", "")
	reqs := []*http.Request{}
	...
	c.Assert(reqs[0].Header.Get("Authorization"), Equals, "token123")
	c.Assert(reqs[0].Header.Get("Token-Owner"), Equals, "tsuru-healer")
}
